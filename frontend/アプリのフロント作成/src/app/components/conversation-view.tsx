...
export function ConversationView() {
  ...
  const { showSentimentAnalysis, showModerationFlags } = useSettings();
  
  const user = users.find((u) => u.id === userId);
  const conversationMessages = messages.filter(
    (m) => (m.senderId === userId && m.receiverId === "current") ||
    ...
    return (
      <div className="flex h-full items-center justify-center p-6">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-800 mb-2">メッセージ機能は利用できません</h2>
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/messages" className="mt-4 inline-block text-teal-600 hover:text-teal-700">
            メッセージ一覧に戻る
          </Link>
        </div>
      </div>
                      ...
                      {showModerationFlags && moderationResult.flags.length > 0 && (
                        <div className="mt-1 flex flex-wrap gap-1 px-2">
                          {moderationResult.flags.map((flag, index) => (
                            <span key={index} className="text-xs text-red-600">⚠️ {flag}</span>
                          ))}
                        </div>
                      )}
